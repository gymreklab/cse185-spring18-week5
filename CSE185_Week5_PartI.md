...
These commands will create an output file `$OUTPREFIX.assoc.logistic`. See the [plink documentation](https://www.cog-genomics.org/plink/1.9/formats#assoc_linear) for a description of each column. There is one row per variant tested. Note depending on parameters used, when run with covariates there will also be an additional line of output for each covariate. Use the script below to pull out only the additive ("ADD") tests for each genotype for downstream analysis (plus keep the header info):
